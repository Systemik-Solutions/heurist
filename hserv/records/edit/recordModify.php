...
function recordUpdateTitle($system, $recID, $rectype_or_mask, $recTitleDefault)
{
    ...
    if(is_numeric($rectype_or_mask) && $rectype_or_mask>0){
        $rectype = intval($rectype_or_mask);
    ...
    if($mask == null) //this is for verification only - it gets mask and rectype in "fill" method
    {
        ...
        if(!$mask){
            $system->addError(HEURIST_DB_ERROR, 'Cannot get title mask for record type '.$rectype.' (Rec#'.$recID.')', $mysqli->error);
            return false;
        }
    }
...
function recordCheckLanguages($system, $recID, $recTypeID){
    ...
    $select = "IF(dtl_Value RLIKE('^[A-Za-z]{2}:'), SUBSTRING(dtl_Value, 1, 2), SUBSTRING(dtl_Value, 1, 3))";
    $issues = [
        'added' => [],
        'skipped' => [],
        'website' => [],
    ...
    if(!$isCMSHome){
        ...
        foreach($textFields as $dtyID){
            ...
            $values = mysql__select_list2($mysqli, "SELECT {$select} FROM recDetails WHERE dtl_RecID = {$recID} AND dtl_DetailTypeID = {$dtyID} AND dtl_Value RLIKE('^[A-Za-z]{2,3}:')");
